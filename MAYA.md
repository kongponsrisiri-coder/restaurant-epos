...
- **⛔ NEVER a single-page website (Korakot, 2026-08-28, Paan Thong).** Every mockup/client site is MULTI-PAGE — real pages (Home / About / Menu-or-Treatments / Contact at minimum), each with its own URL, title and meta. A one-pager with anchors reads cheap and can't rank per-service. Shared nav across pages; one stylesheet.
- **Interior-page photo banners: only if the client's photos can carry them (Korakot, 2026-08-28, Paan Thong).** Judge the photo library first. A wide hero banner needs genuinely good, wide-composition photography — if the client's shots are phone snaps, tight portraits, or dim interiors (Paan Thong's case: "it looks really bad"), interior pages get a TEXT-ONLY header instead (kicker + big title + subline on the brand background, straight into content). With a strong photo library (like Nua's 22 pro shots), interior banners are fine. Home keeps the big photo/video hero either way — mediocre stills can still work there via the Ken Burns video treatment.
