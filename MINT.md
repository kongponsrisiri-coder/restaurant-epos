...
- Shared: **SiamPay** (payments — pitch as COMING, hard rules in MAYA.md) · Inventory · CRM · Staff · Reports · **Website Service £5/mo** · **Social Service £79/mo** (my lane) · **Growth bundle £138/mo** (system + social, website upkeep free)
...
**🖥️ NO DEVICES IN AI SCENES (Korakot, 2026-09-24 — "dont put any device on the photo, its look as ai").** A generated till/laptop/printer reads as fake, and so did a gpt-image swap to OUR real till model. In AI-made scenes, leave devices out and let the props tell the story (food, cables, boxes). Show the till only in Korakot's REAL photos. **Also check CUTLERY and object scale** (same day: an over-long spoon whose handle ran off the plate = "so ai"). Before enqueuing, zoom in on spoons, forks, hands and straws; If the scale still looks wrong after one fix, REMOVE the object; don't keep re-rolling. **Background furniture is the #1 tell** (cloned rows of identical chairs/tables). For prop-led scenes, shoot OVERHEAD (flat lay on the counter): no room means nothing to clone, and it looked real first time (`wk5-visit/hero-overhead.png`) (4 tries on one spoon, 24 Sep, Korakot still saw the wrong ratio). A clean reference of our till (screen blacked out) exists at `social/siamepos/2026-09/wk5-visit/till-ref-blank.jpg` if ever needed.
...
**💷 SOCIAL MEDIA SERVICE = £79/mo (Korakot, 2026-09-26; was £39 inside Growth).** **💷 GROWTH BUNDLE = £138/mo (Korakot, 2026-09-26)** = £59 system + £79 social, with the £5 website upkeep and the £59 social set-up free (£168 on the standard £89). Hardware, the website build and website hosting (the client's own cost) are NOT included. It is the ONLY bundle: the old "£10/month off with any plan" is dead. **£98 and £39 are dead: never quote them anywhere** (posts, cards, sales sheets, reports). Growth-vs-agency card re-rendered at £138: `~/Documents/Claude/Projects/SiamEpos/growth-plan/SiamEPOS-Growth-vs-Agency-card.png`. Current price list + how every product works: https://claude.ai/artifact/FJLShHutJYPqNeaHZ6nYpf

**🥊 WHAT WE'RE UP AGAINST: agency package "Local Growth Solo"** (a restaurant was offered it; Korakot shared it 26 Sep): **£269/mo + ad budget** (Facebook £150/mo, Google £350/mo, paid by the restaurant). Offer: second month free, then 10% off, no contract. Includes ads management · 2 email + 2 SMS campaigns a month (SMS 3.5p each; the restaurant must send its customer list every month) · Google review-request emails + review replies · **4** Facebook/Instagram posts a month · 4 Google Business posts · listing accuracy (Google + Facebook) · a named account manager, results by the **15th** of the next month.
**Our angle:** 8–12 posts vs their 4, at £79 (under a third of £269) · live results in the owner's own back office (Social tab) vs a report by the 15th · the customer list already lives in the system (no monthly spreadsheet) and email campaigns are built in. **Be honest:** they run paid ads and SMS; we don't. Never name their account manager or the restaurant that got the offer.

~~**💷 SiamEPOS Growth = £98/mo (Korakot, 2026-09-25):** £59 system + £39 social, website service included at no charge. Team page https://claude.ai/artifact/GpnHHX7d9oMeEvxigwdQtG. The website BUILD stays a separate one-off (£200, Korakot 25 Sep). Only hosting/updates are in the £98. In posts, still don't quote the build price (rule 11); "quoted separately" is fine. Growth terms: social setup £59 waived, 3-month minimum on the social part (Korakot delegated the call to Mint, 25 Sep).~~ *(SUPERSEDED 26 Sep: social is now £79; bundle price TBC)*
...
| **Highbury Thai Massage** (spa, Highbury) | 🟢 pipe WIRED 2026-09-26 (Krit) — token from **Korakot's page-admin login** (her business portfolio can't approve our share request); verified read + insights; **FIRST POST LIVE 26 Sep 11:51** (Control Room *Approve & post now*; public view verified logged-out) · 3 pending Tue 6/13/20 Oct 18:00 | FB only (203 followers, IG not linked); page id `475088989237978`; **quiet since 14 Dec 2025** | queue with client **`highbury-thai-massage`**, page_key **`HIGHBURY`**; photos: Highbury interior/treatment shoots in `client-sites/`; site highbury-sandy (mockup); brand pink #D60AAA. **Spa rules:** no client faces/names/photos from sessions; **no health or medical claims** (no "cures", "treats", "heals" — say "relax", "ease", "unwind"); treatments + prices only if the owner OKs; CTA = book online / call. First post: Krit checks it from an uninvolved account before any cadence. |
...
**Layout rule (Korakot's v2 catch, same day):** find where the FOOD sits in the photo first and keep it clear; stack the text over the soft/blurred part (usually the top) and anchor the photo with `background-position` so the hero plate sits just above the footer. Never let the headline or fade cover the dish. **No solid colour blocks over the photo (Korakot, 26 Sep: "i dont like that solid part on the upper side")**: prefer full-bleed + a light tint, and choose a photo with calm empty space for the text instead of adding a band. **Pick the layout by photo type:** soft-background shot → `poster-premium.html` (text over the soft area) · overhead flat-lay (food everywhere) → `poster-split.html` (title band on top, framed photo below) · wide landscape table → `poster-band.html` (whole photo as a band on its own blurred copy). Guardrails: real photos only (their posts look AI-edited, which we don't copy); the CTA must be true ("CALL TO BOOK" until online booking/ordering is live on their site); gold-gradient serif is Latin-only, and Thai never goes in gradient text (rule 11).
...
- **Why some client pages "just work" without the client clicking anything (verified 26 Sep):** posting runs on our portfolio's System User **Siamepos Poster** (never expires, not tied to anyone's password). It can only post to pages shared into the Siamepos portfolio. If **Korakot is already an admin of the client's page**, our *Request shared access* is **auto-approved** instantly. That's **Thann Thai**: nobody at the restaurant approved anything, and the page stays client-owned. If the page sits in the **client's own business portfolio** (Highbury), someone in THEIR portfolio must approve. Until then Krit can wire a fallback token from Korakot's page-admin login, which works but dies if Korakot changes his FB password or loses admin. Move it to the System User once they approve.
...
2. **Client photo archives — search the WHOLE `client-sites/<slug>/` folder** (Korakot, 2026-09-24: "you can find it in the client site folder too"). That means `photos/` + `assets/img/` AND every image the built site uses (`site/img/`, `img/`, extracted/archived originals). e.g. Thann Thai 80 images, Baan Rao 117. `find client-sites/<slug> -iname '*.jpg' -o -iname '*.webp' -o -iname '*.png'`. ⚠️ Only for **signed social clients** (most folders are prospects). ⚠️ Skip **AI renders, stock and mockup placeholders** (e.g. The Juicery renders, True Thai stock), check the folder's notes/README first. **Read-only**: copy the file into `social/<slug>/…`, never write back into `client-sites/` (it deploys). Previously listed: — the canonical per-client libraries (Chart Thai has 8 gallery dishes + menu shots; Jinta 17 pro + 6 room; Highbury interior/treatment D85 shoots; Thann Thai webp food shots).
...
**🖼️ IMAGE ENGINE DEFAULT = `gpt-image-2.5-flare` (Korakot, 2026-09-16: "yes go for 2.5-flare").** Bake-off in `marketing/cards/_engine-tests/` (COMPARE-generate.jpg / COMPARE-edit.jpg): on GENERATION 2.5 is ~2× faster and ~¼ the output tokens of gpt-image-2 with a more photographic result; on EDITS (screenshot-on-till, logo-on-product) cost is the same but flare gave the sharpest faithful screen. `2.5-sunburst` = warmer/busier, tends to frame over a person's shoulder — use only for people-led promo scenes. `make-graphic.py` and the weekly `gen-heroes.py` now default to flare. Gotcha: `input_fidelity` is rejected (400) — omit.
...
- `generate "<art prompt>" <out> [gemini]` — seasonal/festival/promo ART (Songkran, Mother's Day…), then composite brand text over it with the HTML method. **Two engines:** default = OpenAI `gpt-image-2.5-flare` (since 2026-09-16 — before that `generate` silently defaulted to Gemini while the header claimed OpenAI; fixed); append `gemini` for Gemini (~4p, more rustic; also the fallback if OpenAI credit runs out). Bake-off reference: `social/siamepos/2026-07/engine-bakeoff.png`. For hero art worth 10p, try both and pick.
...
- **SiamEPOS page = 5 FRESH posts from Mint every week** (⬆️ raised from 3 by Korakot, 2026-08-16: *"i need 5 posts every week"*) — regardless of anything already scheduled; check native Meta Planner posts only to avoid clashing days/slots and repeated topics.
- **Content mix — "only about SiamEPOS is too much":** at most 1 product/promo-led post per week (unchanged at 5/week — the 4 extra slots are ALL knowledge/community, never more promo). Target one post per audience per week: 🌐 digital · 💆 spa · 🛍 retail · 🍽 restaurant · + 1 product OR community/seasonal. The rest = real KNOWLEDGE for Thai business owners in the UK, rotating our three audiences — 🍽 restaurant (allergen law, hygiene ratings, marketplace commission maths, no-shows, seasonal Thai moments — **⛔ NOT tips/service-charge topics, Korakot denied 2026-08-09: "we are do not talking about tips and service charge things"**) · 💆 spa (Treatwell/Fresha economics, deposits, vouchers, loyalty) · 🛍 retail (stock control, card fees, Google Business Profile) · plus general UK small-biz (MTD, hiring, reviews) · 🌐 digital presence — educate WHY a website + active social matter for a business (invisible-on-Google, 35% marketplace commission vs own-site orders, silent page = "closed down", reviews before first visit); our Website £5/mo + Social £79/mo services are the natural, quiet answer. Teach first; at most ONE quiet SiamEPOS line at the end. Verify every law/number before you state it.
- **📰 NEWS STRAND — "ข่าวคนไทยในยูเค" (Korakot, 2026-09-24, after the Nene Royal post):** news that affects Thai people in the UK (or anywhere), **ON TOP OF the 5 weekly posts** (0 in a quiet week, 1–2 when real news happens). Aim = be the Thai page people share, not just a product page. **Topics:** Thai pride (wins, awards, Thai chefs/athletes) · UK rules that hit Thai wallets or paperwork (visas/thresholds, minimum wage, business rates, MTD, card fees) · Thai Embassy London notices (mobile consular services, overseas voting) · travel home (flights, Thai entry rules, baht rate) · UK Thai community dates (Loy Krathong, Songkran, temple fairs). **Visual (Korakot's call):** REAL news photo with a visible credit ("Photo: <owner>" on the card + "📸 ภาพ: <owner>" in the caption), inside OUR graphic frame. Template = `social/siamepos/2026-09/wk4-nene/post-nene-congrats-v3.html`. **Hard rules:** ⛔ NO Thai politics, ⛔ NOTHING about the monarchy (lèse-majesté risk), no tragedy/crime for engagement; ≥2 sources per fact + link the source; no sales pitch (one quiet community line max); post within ~24h, so enqueue with a same-day time and tag the note `📰 NEWS`. **The news image must be readable in one second** (Korakot, 26 Sep, on a dark night-rain photo: "i have no idea what the photo that you put"). If no clear, reusable photo of THIS event exists: make our own data graphic from the official figures (e.g. a province map, template `social/siamepos/2026-09/news-thai-floods/post-news-floods-map.html` with Thai province GeoJSON from chingchai/OpenGISData-Thailand). Never an AI-generated or old photo passed off as current, never an agency press photo without Korakot's OK. Scan daily on the 09:12 run (Thai press, BBC Thai, Thai Embassy London, gov.uk). **Metric = SHARES** (reach isn't available from Meta for our app); review after 4 weeks vs the other posts.
- Daily 09:12 run = maintenance only (denies, schedule_at gaps, photo inbox, quiet-page check) — no weekly drafting there.
- **📧 WEEKLY REPORT — every Sunday ~20:00 run, EMAIL Korakot the weekly social report (Korakot 2026-07-23).** **⚠️ AUTOMATED since ~9 Aug: launchd `com.siamepos.mint-weekly-report` fires Sundays 20:00 and sends it via Brevo — CHECK `~/Library/Application Support/SiamEPOS Control Room/weekly-report.log` for tonight's `SENT` line BEFORE manually sending, or Korakot gets two (Mint duplicated it 30 Aug).** To `kongponsrisiri@gmail.com`. Must be part of the Sunday run (it's LOCAL, so it can read the live `social-queue` + page data — a cloud/scheduled agent CANNOT, don't rely on one). **STRUCTURE = ONE SECTION PER CLIENT** — group by the queue's `client` field / canonical slug, mirroring the Control Room's per-client tabs (Social tab Approval board + Timetable split one pill per `client`). Per client section: **posts PUBLISHED that week on THEIR page** (+ engagement to read on the page), **posts SCHEDULED next week**, **content produced/held**, **flags** (quiet page, low photos, page-access, holds). Then a short cross-client "decisions/actions" + "next-week plan". As the roster grows this is the per-client proof-of-value (and eventually each client gets their OWN report). Today only `siamepos`'s own page is active, so reports are SiamEPOS-only for now. Deliver via **Gmail MCP `create_draft`** (drafts only — no send tool) OR our **Brevo** `sendBrevoEmail` for a true send. Template = first report drafted 2026-07-23 (Gmail draft `r-1862445253909086775`). *(Krit: the Sunday Control-Room-fired Mint run needs Gmail-MCP or BREVO_API_KEY access to deliver this.)*
...
## ⏱ SCHEDULING: POSTS NOW GO VIA FACEBOOK'S OWN SCHEDULER (Mint, 2026-08-16)
**Why:** the Control Room Timetable posts from Korakot's Mac, which is a laptop that sleeps with the lid shut — so an 11:00 slot fired whenever the machine next woke (`kn-shop` 25h late, `kn-foodcost` 2 days late). Confirmed in `pmset -g log`: Clamshell Sleep across both slots.
**The fix:** `~/Library/Application Support/SiamEPOS Control Room/meta-handoff.py`, run by launchd `com.siamepos.mint-meta-handoff` **every 15 min**. It takes items that are **`approved`** with a `schedule_at` **>20 min out**, uploads the photo unpublished, creates a `scheduled_publish_time` feed post, then **clears `schedule_at`** so the Control Room Timetable stands down (its fire condition is `approved AND schedule_at AND schedule_at <= now`). Facebook then publishes on ITS servers at the exact minute — the Mac can be asleep or shut. No edit to Krit's `control-room.py`.
- ⚠️ **Two-step is required:** `/photos` REJECTS `scheduled_publish_time` ("invalid"). Upload unpublished to `/photos` → then `/feed` with `attached_media[0]` + `published=false` + `scheduled_publish_time`.
- **Trade-off to tell Korakot every time:** once handed over, the Timetable no longer controls it. To change time or caption he cancels/edits in **Meta Business Suite → Planner**, or asks me to cancel and re-do. In exchange the post is exact-to-the-minute.
- Fails safe: on ANY error it leaves the item untouched so the Timetable stays the fallback; idempotent via `native_scheduled_id`; never touches pending/denied.
- ⚠️ **The step I first forgot — RECONCILE.** Handing a post to Meta means the Control Room's poster never runs for it, so nothing ever set `status='posted'`. Published posts therefore sat on the Timetable forever as approved-with-no-time, **still showing a `✅ Post now` button that would publish them a second time** (Korakot caught this 18 Aug). `meta-handoff.py` now reconciles on every run: any item whose `native_scheduled_id` has left `scheduled_posts` **and** verifiably exists on the page is marked `posted` + `posted_id`, so it drops off the board exactly like a Timetable post. If it left the scheduler but is NOT on the page it was cancelled by hand in Meta Planner — logged as WARN and left alone, never guessed.
...
- **Naming a competitor with a specific %/£ claim** — ASA risk; generic categories only ("the delivery apps").
- **SiamPay** — nothing public until the first live payout lands (runbook rule).
...
5. Two services stay distinct: **Website £5/mo** (site, booking, ordering, 0% commission) · **Social £79/mo** (FB + IG presence, client approves each post). Bundle = Growth £138/mo (system + social, website upkeep free); build never implied free. Verify prices vs the canonical rate card.
...
11. **NEVER quote a WEBSITE price in a post** (Korakot denied `wk3-kn-saved-replies`, 2026-09-19: *"Dont mention the price as i do for someone at £200, but £5 just for maintenance"*). The £5/mo is the website UPKEEP (maintenance) service; hosting is the client's own cost (Korakot, 26 Sep); the build is quoted case by case (£199–£200+ so far). Writing "£5" reads as the whole website for £5 and undercuts live quotes. Say "เราทำเว็บให้ร้านไทยได้" and stop — price only in DMs. (Social £39/mo may still be stated; it IS the flat price.)
...
- **🎞 REEL STUDIO — talking-head / real-footage reels (built 2026-09-25, Korakot: "do it… build an app").** Double-click `~/Desktop/Reel Studio.command` → http://127.0.0.1:3045. Flow: drop phone clips → brief (goal, length, subtitles TH/EN/both, tone, closing line) → AI transcribes + writes `edit/plan.md` → owner approves or adds notes → cut, graded, **Thai-subtitled 1080×1920** `edit/final.mp4` → revise → *Save to <client> folder* (`social/<client>/<YYYY-MM>/reel-<name>.mp4`). Engine = the open-source **video-use** skill (`~/Developer/video-use`, symlinked to `~/.claude/skills/video-use`, deps in its `.venv` via **uv**, Python 3.14) run headless by `claude -p`. **Needs `ffmpeg-full`** (`/opt/homebrew/opt/ffmpeg-full/bin`): Homebrew's normal ffmpeg has NO libass/drawtext, so subtitles fail. Verified 25 Sep: Thai captions render correctly via font fallback (Thonburi). Transcription = **ElevenLabs Scribe** (supports Thai, rated 'Good'); key in `~/Developer/video-use/.env` (entered in the app, chmod 600). App code: `social/reel-studio/{app.py,index.html}`; projects in `social/reel-studio/projects/`. Posting is still by hand (Business Suite + trending sound) until the pipe handles video. **✨ MOTION ADS (26 Sep):** `reel-factory/make-motion-ad.py` makes Stripe-style product ads (moving brand gradient, data-line motif, sliding white card, staggered text) beat-synced to a CC BY track. Text via Chrome (Pillow here has no raqm, so Thai marks would break). Reference output `social/siamepos/2026-09/ad-motion-oneSystem/`. **Animation frameworks (Remotion / Manim / HyperFrames + parallel sub-agents) NOT set up, by decision (Korakot, 25 Sep: "if you think its enough… keep it as it is").** The current route (PIL + ffmpeg) already does stickers, zooms, speed-ramps and freeze-frames. Only propose Remotion if we need animated charts or motion-graphic ads. Only film Korakot, our team, or people who agreed; no customer faces/names.
