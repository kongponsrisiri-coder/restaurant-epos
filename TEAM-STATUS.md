...
| 2026-05-16 | Krit | Korakot | Desktop installer tag pending. A v1.5.2 build is needed to carry these to the Mac: SEPOS-043, SEPOS-046, BUG-001..005, SEPOS-DELIVERY-002, and now SEPOS-025/026 (printer support — Electron-only, so it ONLY reaches the Mac via a new installer). Say when you want v1.5.2 cut. |
| 2026-05-16 | Krit | Nook | SEPOS-025/026 printer support built + pushed — Electron-only, QA on the Mac desktop app with the cnfujun 80.E-Z04-AA. Test: Admin → Settings → 🖨️ Printer → pick the printer for Receipt + Kitchen → "Test print" prints silently (no dialog). Pay a bill → receipt prints with no dialog. Open an order, add items, hit a 🔥 Fire button → an 80mm kitchen ticket prints. In a plain browser the Printer card shows a "desktop app only" note and receipts still use the print dialog (correct). NOTE: needs the v1.5.2 installer before it's testable on the Mac. |
