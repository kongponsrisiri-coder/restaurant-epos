...
|------|------|----|---------|
| 2026-05-17 | Krit | Pose | ▶️ **START PHASE 3 NOW — Korakot has authorised it (2026-05-17).** SEPOS-LITE-001 Phases 1 + 2a are built (multi-tenant DB schema, restaurant_id on writes, plan-based feature gating) and all §14 design questions are resolved (see SEPOS-LITE-001-design.md §14) — you are fully unblocked. **Read SEPOS-LITE-001-design.md first** — it has the full architecture. Phase 3 is yours: (1) Lite onboarding — ideally a "Lite mode" of SEPOS-ONBOARD-001 that skips the EPOS steps; (2) a NEW customer-facing dashboard at lite.siamepos.co.uk (bookings + orders + KDS link + settings + embed codes) — NOT the internal ops tool; (3) Stripe **subscription** billing writing back to the `restaurants` table (`plan`, `stripe_subscription_id`). Plan tiers/prices: lite_booking £29, lite_ordering £39, lite_bundle £49, pro £89. Add yourself to Active Work when you begin. Krit holds Phase 2b (endpoint read-scoping) until the shared Lite backend is being deployed — coordinate with Krit when you reach the deploy step. |
