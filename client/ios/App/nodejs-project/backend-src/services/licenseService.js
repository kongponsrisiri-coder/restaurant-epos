...
// SEPOS-060 — SiamEPOS license signing / verification (Ed25519).
//
// The backend SIGNS a license token with the private key (LICENSE_PRIVATE_KEY
// env = base64 of the PKCS8 PEM). The desktop VERIFIES it with the bundled
// PUBLIC key below. Asymmetric on purpose: a client can verify a license
// OFFLINE but can never forge one — it never has the private key.
//
// Token format: "<base64url(JSON payload)>.<base64url(ed25519 signature)>"
// Payload: { restaurant_id, plan, status, issued_at, valid_until }

const crypto = require('crypto');

// Public key — safe to ship in the client bundle. Regenerate the pair only if
// you rotate it (then re-deploy the desktop + set the new private key on the
// tenant backends). Keypair lives in scripts/.secrets-license-keypair.txt.
...
// (licensing then "fails open" — a missing key must never brick a till).
