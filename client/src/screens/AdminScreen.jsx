...
function InvoiceScannerTab() {
  ...
  const [editingLine, setEditingLine] = useState(null);    // index of line being edited
  const [confirmResult, setConfirmResult] = useState(null); // { created:[], updated:[], skipped:[] }
  const fileInputRef                  = useRef(null);

  useEffect(() => { invAPI.getIngredients().then(d => setIngredients(Array.isArray(d) ? d : [])); }, []);

  // ── Auto-detect unit from invoice item name ──
  ...
  // ── Fuzzy match extracted name to existing ingredient ──
    ...
    try {
      ...
      if (mode === 'invoice') {
        ...
        setLineItems((inv.line_items || []).map(item => ({
          ...
          unit:                 (item.unit && item.unit !== 'each') ? item.unit : detectUnit(item.name),
  ...
  // ── Update a field on a single line item ──
  const updateLine = (index, field, value) => {
    setLineItems(prev => prev.map((l, i) => {
      if (i !== index) return l;
      const updated = { ...l, [field]: value };
      // auto-recalculate line total when qty or price changes
  ...
  // ── Match dropdown handler ──
  const setMatch = (index, ingredientId) => {
    setLineItems(prev => prev.map((l, i) =>
      i === index ? { ...l, matched_ingredient_id: ingredientId ? Number(ingredientId) : null } : l
    ));
  };

  // ── Add a blank line item manually ──
  const addManualLine = () => {
    const newIndex = lineItems.length;
    setLineItems(prev => [...prev, {
      name_extracted:        'New Item',
      quantity:              1,
      unit:                  'unit',
      unit_price:            0,
      line_total:            0,
    ...
    setEditingLine(newIndex); // open in edit mode immediately
  };
  ...
  // ── CONFIRM INVOICE — auto-creates ingredients for ALL items ──
  const confirmInvoice = async () => {
    if (lineItems.length === 0) return alert('No items to record.');
    setConfirming(true);
    const created = [], updated = [], skipped = [];
    try {
      // fresh fetch of ingredients so we have the latest
      ...
      for (const item of lineItems) {
        ...
        if (!ingredientId) {
          // Check if ingredient already exists (exact name match)
          const existing = existingIngredients.find(ing =>
            ing.name_en.toLowerCase().trim() === item.name_extracted.toLowerCase().trim()
          );
          if (existing) {
            ingredientId = existing.id;
            // Update cost from this invoice
            await invAPI.updateIngredient(existing.id, {
              ...existing,
              cost_per_unit:  item.unit_price,
              supplier_name:  invoiceData.supplier_name || existing.supplier_name,
            });
            updated.push(item.name_extracted);
          } else {
            // Auto-create new ingredient
            const result = await invAPI.addIngredient({
              name_en:          item.name_extracted.trim(),
              name_th:          '',
              unit:             item.unit || detectUnit(item.name_extracted),
              cost_per_unit:    item.unit_price,
            ...
            created.push(item.name_extracted);
          }
        } else {
          // Manually matched to existing — update cost
          const existing = existingIngredients.find(ing => ing.id === ingredientId);
          if (existing) {
            await invAPI.updateIngredient(existing.id, { ...existing, cost_per_unit: item.unit_price });
            updated.push(item.name_extracted);
          }
        }

        // Record stock movement (delivery)
        if (ingredientId) {
      ...
      setConfirmResult({ created, updated, skipped });
      setStage('done');
    } catch (err) {
  ...
  const confirmExpense = async () => {
    ...
    setConfirming(true);
    try {
      await invAPI.addExpense({
        category:    expenseData.category,
        description: `${expenseData.vendor ? expenseData.vendor + ' — ' : ''}${expenseData.description}`,
        amount:      expenseData.total_amount,
        date:        expenseData.date,
      });
      setConfirmResult({ expense: true });
      setStage('done');
    } catch (err) {
      alert('Record failed: ' + err.message);
    } finally {
  ...
  const uploadArea = (
    <div style={{ maxWidth: 580 }}>
      <div style={{
        background: mode === 'invoice' ? '#f0f7ff' : '#fff8f0',
        borderRadius: 12, padding: '14px 18px', marginBottom: 20, fontSize: 13,
        color: mode === 'invoice' ? '#1e40af' : '#92400e',
        border: `1px solid ${mode === 'invoice' ? '#bfdbfe' : '#fed7aa'}`,
      }}>
        {mode === 'invoice'
          ? '💡 Photo your supplier delivery note or invoice. AI reads every line item — items not in your system are auto-created as ingredients.'
          : '💡 Photo any receipt, bill or expense document. AI extracts the cost and auto-categorises it.'}
      </div>
      {error && (
        <div style={{ background: '#fee2e2', borderRadius: 10, padding: '12px 16px', marginBottom: 16, color: '#991b1b', fontSize: 14 }}>
          ⚠️ {error}
        </div>
      )}
      <div
        ...
        style={{
          border: `2px dashed ${file ? '#22c55e' : '#1a1a2e'}`,
    ...
    <div>
      ...
      {stage === 'review' && mode === 'invoice' && (
        <div>
              ...
              {[
                ...
                <div key={f.key}>
                  ...
                  <input
                    ...
                    onChange={e => setInvoiceData(p => ({ ...p, [f.key]: e.target.value }))}
          ...
          {/* Info banner */}
          <div style={{ background: '#f0f7ff', borderRadius: 10, padding: '10px 16px', marginBottom: 12, border: '1px solid #bfdbfe', fontSize: 13, color: '#1e40af' }}>
            💡 <strong>All items auto-create ingredients</strong> if they don't exist yet. Use ✏️ to edit any item, or ➕ to add items AI missed. Match dropdown links to an existing ingredient instead of creating new.
          </div>
          ...
          <div style={{ background: 'white', borderRadius: 12, overflow: 'hidden', boxShadow: '0 1px 4px rgba(0,0,0,0.08)', marginBottom: 16 }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 70px 70px 80px 80px 1fr 80px', padding: '10px 16px', background: '#f8f8f8', fontWeight: 700, fontSize: 12, color: '#555' }}>
              ...
              <span style={{ paddingLeft: 12 }}>Link to Ingredient (optional)</span>
                  ...
                  {isEditing ? (
                    ...
                    <div style={{ padding: '12px 16px', display: 'flex', flexDirection: 'column', gap: 10 }}>
                      <div style={{ display: 'grid', gridTemplateColumns: '1fr 80px 80px 90px 90px', gap: 8 }}>
                        <div>
                          <label style={{ fontSize: 11, fontWeight: 700, color: '#888', display: 'block', marginBottom: 3 }}>Item Name</label>
                          <input value={item.name_extracted} onChange={e => updateLine(i, 'name_extracted', e.target.value)} style={inStyle} />
                        </div>
                        <div>
                          <label style={{ fontSize: 11, fontWeight: 700, color: '#888', display: 'block', marginBottom: 3 }}>Qty</label>
                          <input type="number" step="0.01" value={item.quantity} onChange={e => updateLine(i, 'quantity', e.target.value)} style={inStyle} />
                        </div>
                        <div>
                          <label style={{ fontSize: 11, fontWeight: 700, color: '#888', display: 'block', marginBottom: 3 }}>Unit</label>
                          <select value={item.unit} onChange={e => updateLine(i, 'unit', e.target.value)} style={inStyle}>
                            {['kg', 'g', 'L', 'ml', 'unit', 'each'].map(u => <option key={u} value={u}>{u}</option>)}
                          </select>
                        </div>
                        <div>
                          <label style={{ fontSize: 11, fontWeight: 700, color: '#888', display: 'block', marginBottom: 3 }}>Unit Price £</label>
                          <input type="number" step="0.01" value={item.unit_price} onChange={e => updateLine(i, 'unit_price', e.target.value)} style={inStyle} />
                        </div>
                        <div>
                          <label style={{ fontSize: 11, fontWeight: 700, color: '#888', display: 'block', marginBottom: 3 }}>Line Total £</label>
                          <input type="number" step="0.01" value={item.line_total} onChange={e => updateLine(i, 'line_total', e.target.value)} style={inStyle} />
                        </div>
                      </div>
                      <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                        <div style={{ flex: 1 }}>
                          <label style={{ fontSize: 11, fontWeight: 700, color: '#888', display: 'block', marginBottom: 3 }}>Link to existing ingredient (optional — leave blank to auto-create)</label>
                          <select value={item.matched_ingredient_id || ''} onChange={e => setMatch(i, e.target.value || null)} style={inStyle}>
                            <option value="">🤖 Auto-create new ingredient</option>
                            {ingredients.map(ing => <option key={ing.id} value={ing.id}>{ing.name_en}</option>)}
                          </select>
                        </div>
                      ...
                      <div style={{ paddingLeft: 12 }}>
                        <select
                          ...
                          style={{
                            ...
                            border: `1px solid ${isMatched ? '#22c55e' : '#bfdbfe'}`,
                            background: isMatched ? '#f0fdf4' : '#f0f7ff',
                            color: isMatched ? '#14532d' : '#1e40af',
                          }}
                        >
            ...
            <div style={{ padding: '10px 16px', borderTop: '2px dashed #e0e0e0', display: 'flex', justifyContent: 'center' }}>
              <button onClick={addManualLine} style={{ padding: '8px 20px', borderRadius: 8, border: '2px dashed #3b82f6', background: 'white', color: '#3b82f6', fontWeight: 700, cursor: 'pointer', fontSize: 13 }}>
                ➕ Add Item Manually
              </button>
            </div>
          </div>
          ...
          <div style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
            <div style={{ flex: 1, fontSize: 13, color: '#555' }}>
              <span style={{ color: '#1a1a2e', fontWeight: 700 }}>📦 {lineItems.length} item{lineItems.length !== 1 ? 's' : ''} to record</span>
              <span style={{ color: '#888', marginLeft: 8, fontSize: 12 }}>— new ingredients will be auto-created</span>
            </div>
            ...
            <button
              ...
              style={{
                padding: '12px 28px', borderRadius: 10, border: 'none', fontWeight: 700, fontSize: 15,
                cursor: confirming || lineItems.length === 0 ? 'default' : 'pointer',
                background: confirming || lineItems.length === 0 ? '#ddd' : '#1a1a2e',
                color: 'white',
              }}
            >
              {confirming ? 'Recording...' : `✓ Confirm & Record ${lineItems.length} Item${lineItems.length !== 1 ? 's' : ''}`}
            </button>
          </div>
        </div>
      )}
              ...
              {confirmResult.created?.length > 0 && (
                <div style={{ marginBottom: 12 }}>
                  <div style={{ fontWeight: 700, fontSize: 13, color: '#22c55e', marginBottom: 6 }}>
                    🆕 {confirmResult.created.length} new ingredient{confirmResult.created.length > 1 ? 's' : ''} created:
                  </div>
                  {confirmResult.created.map(n => (
                    <div key={n} style={{ fontSize: 13, color: '#555', padding: '2px 0', paddingLeft: 12 }}>• {n}</div>
                  ))}
                </div>
              )}
              {confirmResult.updated?.length > 0 && (
                <div>
                  <div style={{ fontWeight: 700, fontSize: 13, color: '#3b82f6', marginBottom: 6 }}>
                    🔄 {confirmResult.updated.length} ingredient{confirmResult.updated.length > 1 ? 's' : ''} cost updated:
                  </div>
                  {confirmResult.updated.map(n => (
                    <div key={n} style={{ fontSize: 13, color: '#555', padding: '2px 0', paddingLeft: 12 }}>• {n}</div>
                  ))}
                </div>
              )}
              <div style={{ marginTop: 12, fontSize: 12, color: '#888', borderTop: '1px solid #f0f0f0', paddingTop: 10 }}>
                📦 All quantities recorded in Stock Log · Go to <strong>📋 Recipes & Costs</strong> to build recipes using these ingredients
              </div>
            </div>
          )}

          {confirmResult?.expense && (
    ...
    <div style={{ padding: 24 }}>
      ...
      <div style={{ display: 'flex', gap: 16, marginBottom: 16, flexWrap: 'wrap' }} className="no-print">
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 12, color: '#555' }}>
          <span style={{ display: 'inline-flex', width: 24, height: 24, borderRadius: 6, background: '#ef4444', color: 'white', fontSize: 14, fontWeight: 900, alignItems: 'center', justifyContent: 'center' }}>✓</span>
          Red = Contains (auto from recipe)
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 12, color: '#555' }}>
          <span style={{ display: 'inline-flex', width: 24, height: 24, borderRadius: 6, background: '#eab308', color: 'white', fontSize: 14, fontWeight: 900, alignItems: 'center', justifyContent: 'center' }}>✓</span>
          Yellow = Contains (set manually)
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 12, color: '#555' }}>
      ...
      {loading ? (
        ...
        /*
         * FIX 1: maxHeight + overflow:auto creates a contained scroll zone
         *        so position:sticky on <th> works relative to THIS box.
         * FIX 2: borderCollapse:'separate' is REQUIRED for position:sticky
         *        on thead <th> elements to actually stick.
         */
        <div
          className="allergen-table-wrap"
          style={{
            background: 'white',
            borderRadius: 12,
            overflow: 'auto',
        ...
        >
          ...
          <table style={{ width: '100%', borderCollapse: 'separate', borderSpacing: 0, fontSize: 11 }}>
            <thead>
              <tr>
                {/* DISH — sticky top AND sticky left (corner cell needs highest z-index) */}
                <th style={{
                  padding: '14px 16px',
                  textAlign: 'left',
                  color: 'white',
                      ...
                      return (
                        <tr key={item.id} style={{ background: rowBg }}>
                          ...
                          <td style={{
                            ...
                            {item.name_alt && (
                              <div style={{ fontSize: 10, color: '#C9A84C', marginTop: 1 }}>{item.name_alt}</div>
                            )}
                            {isSaving && (
                              <div style={{ fontSize: 9, color: '#aaa', marginTop: 2 }}>saving...</div>
                            )}
                          </td>
                              ...
                              >
                                {contains ? (
                                  ...
                                  <span style={{
                                    ...
                                    height: 26,
                                    borderRadius: 6,
                                    background: fromRecipe ? '#ef4444' : '#eab308',
                                    color: 'white',
                                    fontSize: 15,
                                    fontWeight: 900,
                                    alignItems: 'center',
                          ...
                          <td style={{ padding: '9px 8px', whiteSpace: 'nowrap', borderBottom: '1px solid #f3f3f3' }} className="no-print">
                            {fromRecipe ? (
                              <span style={{ fontSize: 11, color: '#22c55e', fontWeight: 700 }}>✅ recipe</span>
                            ) : source === 'manual' ? (
                              <span style={{ fontSize: 11, color: '#eab308', fontWeight: 700 }}>✏️ manual</span>
                            ) : (
