...
export default function BillScreen({ orderId, onClose, onPay }) {
  ...
  const [paymentDetails, setPaymentDetails] = useState(null); // set after confirm
  ...
  const getPersonTotal = (personIdx) => {
    ...
    return {
      items: personItems, subtotal: personSubtotal, discount: personDiscount,
      afterDiscount: personAfterDiscount, service: personService,
  ...
  const handleNumpad = (btn) => {
    ...
    if (btn === '.') {
      ...
      setPaymentInput(prev => (prev || '0') + '.'); return;
    }
  ...
  const handleConfirmPayment = () => {
    ...
    // Save payment details and go to receipt stage (don't record yet)
    setPaymentDetails({
  ...
  // Called from receipt stage — records payment and closes
  const handleFinish = () => {
  ...
  const handleSplitEqualPayment = (index) => {
    ...
    if (newPaid.length >= splitCount) {
      setPaymentDetails({ method: 'Split', amountPaid: billTotal, tip: 0, change: 0 });
      setStage('receipt');
    }
  };
  ...
  const handleSplitItemPayment = (personIdx) => {
    ...
    if (newPaid.length >= splitItemCount) {
      setPaymentDetails({ method: 'Split by Item', amountPaid: billTotal, tip: 0, change: 0 });
      setStage('receipt');
    }
  };
  ...
  const overlay = {
    ...
    background: isMobile ? 'white' : 'rgba(0,0,0,0.75)',
    display: 'flex',
  ...
  const card = {
    ...
    width: '100%',
    maxWidth: isMobile ? '100%' : (stage === 'amount' ? 820 : stage === 'split_items' ? 600 : 480),
  ...
  return (
    <div style={overlay}>
      <div style={card}>
          ...
          <div>
            ...
            <div style={{ padding: isMobile ? '20px 16px' : '28px 24px' }}>
              <div style={{ textAlign: 'center', marginBottom: 20 }}>
                <div style={{ fontSize: 22, fontWeight: 800, color: '#1a1a2e' }}>{settings.company_name || 'My Restaurant'}</div>
              ...
              <div style={{ marginBottom: 16, fontFamily: 'monospace' }}>
                {billItems.map(item => {
                  ...
                  const itemDiscount = item.discount_value > 0
                    ? item.discount_type === 'percent' ? itemPrice * (item.discount_value / 100) : Math.min(item.discount_value, itemPrice)
                    : 0;
                    ...
                    <div key={item.id} style={{ marginBottom: 8 }}>
                      ...
                      {item.discount_value > 0 && (
                        <div style={{ fontSize: 11, color: '#22c55e', marginLeft: 16 }}>
                          🏷️ {item.discount_type === 'percent' ? `${item.discount_value}% off` : `£${item.discount_value} off`} (-£{itemDiscount.toFixed(2)})
                        </div>
                      )}
                    </div>
                  );
              ...
              <div style={{ borderTop: '1px dashed #ccc', paddingTop: 12, marginBottom: 16 }}>
                ...
                {discountAmount > 0 && (
                  <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 14, marginBottom: 6, color: '#22c55e' }}>
                    <span>Discount ({order.discount_reason})</span><span>-£{discountAmount.toFixed(2)}</span>
                  </div>
                )}
                {serviceChargeEnabled && (
                  <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 14, marginBottom: 6, color: '#555' }}>
                    <span>Service charge ({settings.service_charge_percent || 12.5}%)</span><span>£{serviceCharge.toFixed(2)}</span>
                  </div>
                )}
            ...
            <div style={{ padding: isMobile ? '0 16px 32px' : '0 24px 24px', display: 'flex', flexDirection: 'column', gap: 10 }}>
              <button onClick={() => setStage('method')} style={{ padding: '18px', borderRadius: 12, border: 'none', background: '#1a1a2e', color: 'white', fontSize: 18, fontWeight: 800, cursor: 'pointer' }}>
                💳 Take Payment — £{billTotal.toFixed(2)}
              </button>
              <button onClick={() => { setSplitPaid([]); setStage('split_equal'); }} style={{ padding: '14px', borderRadius: 12, border: '2px solid #C9A84C', background: 'white', color: '#C9A84C', fontSize: 15, fontWeight: 700, cursor: 'pointer' }}>
                ✂️ Split Equally
              </button>
              <button onClick={() => { setItemAssignments({}); setSplitItemPaid([]); setActivePerson(0); setStage('split_items'); }} style={{ padding: '14px', borderRadius: 12, border: '2px solid #3b82f6', background: 'white', color: '#3b82f6', fontSize: 15, fontWeight: 700, cursor: 'pointer' }}>
                🍽️ Split by Item
              </button>
              {/* Print Bill — thermal receipt without payment details */}
              <button onClick={handlePrintBill} style={{ padding: '12px', borderRadius: 10, border: '2px solid #1a1a2e', background: 'white', color: '#1a1a2e', fontSize: 14, fontWeight: 600, cursor: 'pointer' }}>
                🖨️ Print Bill
              </button>
              {!isMobile && (
                <button onClick={onClose} style={{ padding: '12px', borderRadius: 10, border: 'none', background: '#f0f0f0', color: '#555', fontSize: 14, cursor: 'pointer' }}>Close</button>
              )}
            </div>
          </div>
        )}
        ...
        {/* ══════════════════════════════════════════════════════════
            STAGE: RECEIPT (after payment confirmed)
            ══════════════════════════════════════════════════════════ */}
        {stage === 'receipt' && (
          <div>
            ...
            <div style={{ padding: isMobile ? '32px 20px' : 40, textAlign: 'center' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 12, maxWidth: 340, margin: '0 auto' }}>
                <button onClick={handlePrintReceipt}
                  style={{ padding: '16px 24px', borderRadius: 12, border: '2px solid #1a1a2e', background: 'white', color: '#1a1a2e', fontSize: 16, fontWeight: 700, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8 }}>
                  🖨️ Print Receipt
                </button>
                <button onClick={handleFinish}
                  style={{ padding: '18px 24px', borderRadius: 12, border: 'none', background: '#1a1a2e', color: 'white', fontSize: 17, fontWeight: 800, cursor: 'pointer' }}>
                  ✓ Done — Close
                </button>
              </div>

              <p style={{ fontSize: 11, color: '#bbb', marginTop: 20 }}>
                Printer not shown? Set your thermal printer as the default printer in your browser settings.
              </p>
            </div>
          </div>
        )}
        ...
        {stage === 'split_equal' && (
          <div>
            ...
            <div style={{ padding: isMobile ? '20px 16px' : 32 }}>
              ...
              {isMobile && (
                <div style={{ textAlign: 'center', marginBottom: 20 }}>
                  <div style={{ fontSize: 15, color: '#888' }}>Total: <span style={{ fontWeight: 800, color: '#1a1a2e', fontSize: 20 }}>£{billTotal.toFixed(2)}</span></div>
                </div>
              )}
              ...
              <div style={{ background: '#f8f8f8', borderRadius: 14, padding: 20, marginBottom: 24 }}>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 13, color: '#888', marginTop: 8 }}>
                  <span>Paid so far</span><span>{paidCount} of {splitCount} people</span>
                </div>
                {paidCount > 0 && (
                  <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 14, color: '#22c55e', marginTop: 8, fontWeight: 700 }}>
                    <span>Remaining</span><span>£{remainingEqual.toFixed(2)}</span>
                  </div>
                )}
              </div>
                ...
                {Array.from({ length: splitCount }, (_, i) => {
                  ...
                  return (
                    <div key={i} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '14px 18px', borderRadius: 12, background: isPaid ? '#f0fdf4' : 'white', border: `2px solid ${isPaid ? '#22c55e' : '#e0e0e0'}` }}>
                      <div>
                        <div style={{ fontWeight: 700, color: isPaid ? '#22c55e' : '#1a1a2e' }}>{isPaid ? '✅' : '👤'} Person {i + 1}</div>
            ...
            <div style={{ padding: isMobile ? '16px 16px' : 28 }}>
              ...
              <div style={{ marginBottom: 16 }}>
                ...
                <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                  {Array.from({ length: splitItemCount }, (_, i) => (
                    <button key={i} onClick={() => setActivePerson(i)} style={{ padding: isMobile ? '12px 16px' : '10px 16px', borderRadius: 8, border: 'none', cursor: 'pointer', fontWeight: 700, fontSize: 13, background: activePerson === i ? personColors[i] : '#f0f0f0', color: activePerson === i ? 'white' : '#555', opacity: splitItemPaid.includes(i) ? 0.4 : 1 }}>
                      👤 Person {i + 1}
                    </button>
                  ))}
                </div>
              ...
              <div style={{ marginBottom: 20 }}>
                ...
                {billItems.map(item => {
                  ...
                  return (
                    <div key={item.id}
                      onClick={() => { if (splitItemPaid.includes(assignedTo)) return; setItemAssignments(prev => ({ ...prev, [item.id]: activePerson })); }}
                      style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: isMobile ? '14px 16px' : '12px 16px', borderRadius: 10, marginBottom: 8, border: `2px solid ${isAssigned ? assignedColor : '#e0e0e0'}`, background: isAssigned ? `${assignedColor}15` : 'white', cursor: 'pointer' }}>
                      <div style={{ flex: 1 }}>
                        ...
                        {item.discount_value > 0 && <div style={{ fontSize: 11, color: '#22c55e' }}>🏷️ -£{itemDiscount.toFixed(2)}</div>}
                      </div>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                        <span style={{ fontWeight: 700, color: '#1a1a2e' }}>£{itemTotal.toFixed(2)}</span>
                        {isAssigned
                          ? <div style={{ background: assignedColor, color: 'white', borderRadius: 20, padding: '3px 10px', fontSize: 11, fontWeight: 700 }}>P{assignedTo + 1}</div>
                      ...
                      <div key={i} style={{ borderRadius: 12, padding: 16, marginBottom: 10, border: `2px solid ${isPaid ? '#22c55e' : personColors[i]}`, background: isPaid ? '#f0fdf4' : `${personColors[i]}08` }}>
                        ...
                        {p.items.map(item => (
                          <div key={item.id} style={{ display: 'flex', justifyContent: 'space-between', fontSize: 12, color: '#555', marginBottom: 2 }}>
                            <span>{item.quantity}× {item.name}</span><span>£{(item.unit_price * item.quantity).toFixed(2)}</span>
                          </div>
                        ))}
                        <div style={{ borderTop: '1px dashed #ccc', marginTop: 8, paddingTop: 8 }}>
                          ...
                          {serviceChargeEnabled && p.service > 0 && <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 12, color: '#555', marginBottom: 3 }}><span>Service ({settings.service_charge_percent || 12.5}%)</span><span>£{p.service.toFixed(2)}</span></div>}
                        </div>
                        {!isPaid && (
            ...
            <div style={{ padding: isMobile ? '24px 16px 32px' : 32 }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 12, marginBottom: 16 }}>
                {[{ method: 'Cash', icon: '💵' }, { method: 'Card', icon: '💳' }, { method: 'Other', icon: '🔄' }].map(({ method, icon }) => (
                  <button key={method} onClick={() => { setSelectedMethod(method); setPaymentInput(''); setStage('amount'); }} style={{ padding: isMobile ? '22px' : '20px', borderRadius: 12, border: '2px solid #1a1a2e', background: 'white', color: '#1a1a2e', fontSize: isMobile ? 22 : 20, fontWeight: 700, cursor: 'pointer' }}>
                    {icon} {method}
                  </button>
                ))}
              </div>
            ...
            {isMobile && (
              <div style={{ display: 'flex', flexDirection: 'column', minHeight: '100vh' }}>
                ...
                <div style={{ background: '#0D1B3E', padding: '20px 20px 16px', display: 'flex', flexDirection: 'column' }}>
                  ...
                  {canPay && selectedMethod === 'Cash' && change > 0 && (
                    <div style={{ marginTop: 8, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                      <span style={{ fontSize: 13, color: 'rgba(255,255,255,0.5)' }}>Change to give</span>
                      <span style={{ fontSize: 22, fontWeight: 800, color: '#22c55e' }}>£{change.toFixed(2)}</span>
                    </div>
                  )}
                  {canPay && actualTip > 0 && selectedMethod !== 'Cash' && (
                    <div style={{ marginTop: 8, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                      <span style={{ fontSize: 13, color: 'rgba(255,255,255,0.5)' }}>Tip</span>
                      <span style={{ fontSize: 22, fontWeight: 800, color: '#8b5cf6' }}>£{actualTip.toFixed(2)}</span>
                    </div>
                  )}
                </div>
                ...
                <div style={{ padding: '8px 16px' }}>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 8 }}>
                    {[billTotal, Math.ceil(billTotal/5)*5, Math.ceil(billTotal/10)*10, Math.ceil(billTotal/20)*20].filter((v,i,arr) => arr.indexOf(v) === i).map(amount => (
                ...
                <div style={{ flex: 1, minWidth: 280, padding: 28, borderRight: '1px solid #eee' }}>
                  ...
                  <div style={{ background: '#f8f8f8', borderRadius: 12, padding: 16, marginBottom: 20 }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 15, color: '#555', marginBottom: 10 }}>
                      <span>Bill total</span><span style={{ fontWeight: 700 }}>£{billTotal.toFixed(2)}</span>
                    </div>
                    <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 15, color: '#555', marginBottom: 10 }}>
                      <span>Amount received</span><span style={{ fontWeight: 800, color: '#1a1a2e', fontSize: 18 }}>£{amountPaid > 0 ? amountPaid.toFixed(2) : '—'}</span>
                    </div>
                    {canPay && selectedMethod === 'Cash' && (
                      <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 20, fontWeight: 800, color: '#22c55e', borderTop: '2px solid #eee', paddingTop: 10, marginTop: 4 }}>
                        <span>💚 Change</span><span>£{Math.max(0, change).toFixed(2)}</span>
                      </div>
                    )}
                    {canPay && amountPaid > billTotal && selectedMethod !== 'Cash' && (
                      <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 20, fontWeight: 800, color: '#8b5cf6', borderTop: '2px solid #eee', paddingTop: 10, marginTop: 4 }}>
                        <span>💜 Tip</span><span>£{actualTip.toFixed(2)}</span>
                      </div>
                    )}
                  </div>
                  <div style={{ marginBottom: 20 }}>
