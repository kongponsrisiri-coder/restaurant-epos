...
function Timer({ startedAt }) {
  ...
  return (
    <div style={{ fontSize: 20, fontWeight: 800, color, fontFamily: 'monospace', minWidth: 60, textAlign: 'right' }}>
...
export default function KitchenScreen() {
  ...
  if (data.order) {
    ...
    // Play different sound per course
  ...
  const passOrders = orders
    .map(o => ({
      ...o,
      items: o.items.filter(i => !i.voided && i.status !== 'served' && !i.is_bar)
    }))
    ...
    <div style={{ background: '#111', height: '100vh', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
      ...
      <div style={{ padding: '8px 20px', display: 'flex', gap: 24, borderBottom: '1px solid #222', flexWrap: 'wrap', flexShrink: 0 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
          ...
          <span style={{ color: '#aaa', fontSize: 12 }}>📋 Upcoming</span>
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
        ...
        <div style={{
          ...
          {orders.length === 0 ? (
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(300px, 1fr))', gap: 16 }}>
              {orders.map(order => {
                const allItems = order.items.filter(i =>
                  !i.voided && i.status !== 'served' && i.status !== 'cooked' && !i.is_bar
                );
                const upcoming = allItems.filter(i => !i.is_fired);
                const cooking = allItems.filter(i =>
                  i.is_fired && (filter === 'all' || String(i.course || 1) === filter)
                );
                    ...
                    <div style={{ padding: 12 }}>
                      ...
                      {upcoming.length > 0 && (
                        <div style={{ marginBottom: 12 }}>
                          <div style={{ color: '#6b7280', fontSize: 11, fontWeight: 700, textTransform: 'uppercase', marginBottom: 8, letterSpacing: 1 }}>
                            📋 Upcoming — waiting to be fired
                          </div>
                          {(() => {
                            const byCourse = {};
                            ...
                            return Object.keys(byCourse).sort().map(course => (
                              <div key={course} style={{ marginBottom: 8 }}>
                                ...
                                {byCourse[course].map(item => (
                                  <div key={item.id} style={{ background: '#2a2a2a', borderRadius: 8, padding: '8px 12px', marginBottom: 4, border: '1px solid #374151' }}>
                                    <div style={{ color: '#9ca3af', fontWeight: 600, fontSize: 14 }}>
                                      {item.quantity}× {item.name}
                                    </div>
                                    {item.notes && <div style={{ color: '#6b7280', fontSize: 12, marginTop: 2 }}>{item.notes}</div>}
                                  ...
                                  <div key={item.id} style={{ background: '#1e3a5f', borderRadius: 10, padding: '12px', marginBottom: 8, border: '2px solid #3b82f6' }}>
                                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                      <div style={{ flex: 1 }}>
                                        ...
                                        {item.notes && <div style={{ color: '#93c5fd', fontSize: 12, marginTop: 2 }}>{item.notes}</div>}
      ...
      {tab === 'pass' && (
        <div style={{
          ...
          {passOrders.length === 0 ? (
            <div style={{ textAlign: 'center', color: '#555', marginTop: 100, fontSize: 20 }}>
              ✅ Pass is clear — nothing to plate up!
            </div>
          ) : (
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(300px, 1fr))', gap: 16 }}>
                          ...
                          {byCourse[course].map(item => {
                            ...
                            return (
                              <div key={item.id} style={{
                                background: isCooked ? '#2d1f4e' : '#2a2a2a',
                                borderRadius: 10, padding: '12px', marginBottom: 8,
                                border: `2px solid ${isCooked ? '#8b5cf6' : isUpcoming ? '#374151' : '#1e3a5f'}`,
                                opacity: isUpcoming ? 0.5 : 1,
                              }}>
                                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                  <div style={{ flex: 1 }}>
                                    <div style={{ color: isCooked ? 'white' : '#9ca3af', fontWeight: 700, fontSize: 15 }}>
                                      {item.quantity}× {item.name}
                                    </div>
                                  ...
                                  <div style={{ marginLeft: 12, flexShrink: 0, textAlign: 'center' }}>
                                    ...
                                    {isCooking && (
                                      <div style={{ color: '#3b82f6', fontSize: 12, fontWeight: 700, textAlign: 'center' }}>
                                        🔥<br />Cooking
                                      </div>
                                    )}
                                    {isUpcoming && (
                                      <div style={{ color: '#4b5563', fontSize: 12, fontWeight: 600, textAlign: 'center' }}>
                                        📋<br />Upcoming
                                      </div>
                                    )}
                                  </div>
      ...
      {tab === 'completed' && (
        <div style={{
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
            ...
            <button onClick={fetchCompleted} style={{ background: '#333', color: 'white', border: 'none', padding: '8px 14px', borderRadius: 8, cursor: 'pointer', fontSize: 13 }}>
              Refresh
            </button>
          </div>
