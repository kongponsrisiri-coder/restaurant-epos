 ...
 * Totals are passed in from BillScreen — no recalculation here.
 */
...
function buildReceiptHTML({ order, items, settings, paymentDetails }) {
  ...
  // ── Totals: use pre-calculated values passed from BillScreen ──
  ...
  const paymentRows = method ? `
    ...
    <tr>
      <td>Cash tendered</td>
      <td style="text-align:right;">${fmt(amountPaid)}</td>
    </tr>
  ...
  const qrSection = googleReviewUrl ? `
    <div style="text-align:center;margin-top:10px;">
      ...
      <img src="https://api.qrserver.com/v1/create-qr-code/?size=80x80&data=${encodeURIComponent(googleReviewUrl)}"
           width="80" height="80" alt="QR" style="border:1px solid #eee;" />
...
<head>
  ...
  <style>
    * { margin:0; padding:0; box-sizing:border-box; }
    ...
    h1 { font-size:16px; font-weight:900; text-align:center; letter-spacing:1px; margin-bottom:2px; }
    table { width:100%; border-collapse:collapse; }
    .total-row td { padding:4px 0; border-top:2px solid #000; font-size:15px; font-weight:900; }
    .thank-you { text-align:center; font-size:11px; margin-top:10px; color:#333; }
  </style>
</head>
<body>
  <h1>${restaurantName}</h1>
  ${restaurantAddr  ? `<p class="center" style="font-size:10px;color:#555;">${restaurantAddr}</p>` : ''}
  ${restaurantPhone ? `<p class="center" style="font-size:10px;color:#555;">Tel: ${restaurantPhone}</p>` : ''}
  ...
  <table>
    ...
    <tr class="total-row">
      <td>TOTAL</td>
      <td style="text-align:right;">${fmt(billTotal)}</td>
    </tr>
    ${paymentRows}
  </table>
