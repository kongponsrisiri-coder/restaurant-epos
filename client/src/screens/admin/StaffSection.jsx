...
export default function StaffSection() {
  ...
  const [form, setForm]             = useState({ name: '', pin: '', role: 'waiter', email: '', start_date: '', notes: '', employment_status: 'active', can_discount: 0, can_redeem_deposit: 0, can_void: 0, can_close_z: 0 });
            ...
            {selectedStaff?.id === s.id && (
              <div style={{ background: '#f8f8f8', borderRadius: '0 0 12px 12px', padding: '16px 20px', border: '2px solid #e94560', borderTop: 'none' }}>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 12 }}>
                  {[{ label: 'Start Date', value: s.start_date || '—' }, { label: 'Email', value: s.email || '—' }, { label: 'Status', value: s.employment_status || 'Active' }, { label: 'Member Since', value: s.created_at ? new Date(s.created_at).toLocaleDateString('en-GB', { day: '2-digit', month: 'short', year: 'numeric' }) : '—' }, { label: 'Role', value: s.role }].map(item => (
        ...
        <div style={{ position: 'fixed', top: 0, right: 0, bottom: 0, left: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }}>
          <div style={{ background: 'white', borderRadius: 16, padding: 32, width: 420, maxHeight: '90vh', overflowY: 'auto' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
              ...
              <div><label style={{ fontSize: 13, fontWeight: 600, color: '#555', display: 'block', marginBottom: 6 }}>Email <span style={{ fontWeight: 400, color: '#aaa' }}>(optional)</span></label><input type="email" value={form.email} onChange={e => setForm({ ...form, email: e.target.value })} placeholder="owner@restaurant.co.uk" style={{ width: '100%', padding: '10px 12px', borderRadius: 8, border: '1px solid #ddd', fontSize: 14, boxSizing: 'border-box' }} />{['admin','manager','supervisor'].includes(form.role) && <div style={{ fontSize: 12, color: '#888', marginTop: 5 }}>Used for owner sign-in links and to authorise new till devices.</div>}</div>
              ...
              {/* SEPOS-STAFF-PERMS-001 — grant these to a trusted non-manager without making them a manager. */}
              <div style={{ background: '#f9fafb', border: '1px solid #eee', borderRadius: 10, padding: '12px 14px' }}>
                <label style={{ fontSize: 13, fontWeight: 700, color: '#555', display: 'block', marginBottom: 8 }}>Permissions <span style={{ fontWeight: 400, color: '#aaa' }}>(managers can always do these)</span></label>
