  ...
  return (
    <div>
      <div style={{ background: 'white', borderRadius: 12, padding: 20, marginBottom: 20, boxShadow: '0 1px 4px rgba(0,0,0,0.08)' }}>
        <div style={{ fontWeight: 700, fontSize: 15, color: 'var(--brand-primary, #1a1a2e)', marginBottom: 14 }}>📝 Record Stock Movement</div>
        <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', alignItems: 'flex-end' }}>
          ...
          <button onClick={handleAdjust} disabled={saving} style={{ padding: '10px 20px', borderRadius: 8, border: 'none', background: 'var(--brand-primary, #1a1a2e)', color: 'white', fontWeight: 700, cursor: 'pointer', whiteSpace: 'nowrap' }}>{saving ? 'Saving...' : 'Record'}</button>
        </div>
      ...
      <div style={{ display: 'flex', gap: 8, marginBottom: 14, alignItems: 'center', flexWrap: 'wrap' }}>
        {MOVEMENT_TYPES.map(t => (<button key={t.id} onClick={() => setFilterType(t.id)} style={{ padding: '6px 14px', borderRadius: 20, border: 'none', cursor: 'pointer', fontWeight: 600, fontSize: 13, background: filterType === t.id ? 'var(--brand-primary, #1a1a2e)' : '#e0e0e0', color: filterType === t.id ? 'white' : '#555' }}>{t.label}</button>))}
        <button onClick={exportCsv} disabled={!filtered.length} style={{ marginLeft: 'auto', padding: '6px 14px', borderRadius: 8, border: '1px solid var(--brand-primary, #1a1a2e)', background: 'white', color: 'var(--brand-primary, #1a1a2e)', fontWeight: 700, fontSize: 12, cursor: filtered.length ? 'pointer' : 'not-allowed', opacity: filtered.length ? 1 : 0.5 }}>⬇ Export CSV</button>
      </div>
              ...
              <div key={m.id || i} style={{ display: 'grid', gridTemplateColumns: '130px 1fr 90px 80px 1fr', padding: '10px 16px', borderBottom: '1px solid #f0f0f0', fontSize: 13, alignItems: 'center' }}>
                ...
                <div>
                  {isOrphaned
                    ...
                    : <div style={{ fontWeight: 600, color: 'var(--brand-primary, #1a1a2e)' }}>{displayName}</div>
                  }
