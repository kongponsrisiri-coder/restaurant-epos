...
// (applyBrandTheme). Screens import these tokens — which resolve to the CSS
// var with the default SiamEPOS colour baked in as fallback — so a tenant's
// theme repaints the whole POS. With no override, the vars fall back to the
// current navy/gold, so existing clients look identical (zero change).
//
...
// Brand tokens — reference the CSS var, default baked in for when it's unset.
export const NAVY  = `var(--brand-primary, ${DEFAULT_PRIMARY})`;
export const GOLD  = `var(--brand-accent, ${DEFAULT_ACCENT})`;
...
// the UI. Call this once settings are loaded (and after a branding save).
