...
| BUG-EPOS-002 | `POST /api/orders/:id/items` → 500 when `unit_price` missing from payload; server should fetch price from DB server-side | Restaurant EPOS | High | 🔴 Open | Krit | 2026-06-02 |
...
| BUG-EPOS-004 | `window.confirm()` silently fails throughout app in Electron — confirmations do nothing on desktop | Restaurant EPOS | High | 🔴 Open | Krit | 2026-06-03 |
| BUG-EPOS-005 | `/api/reports/daily` excludes service charge from totals — ~11% undercount every day | Restaurant EPOS | Medium | 🔴 Open | Krit | 2026-06-03 |
| BUG-EPOS-006 | Discount type picker always selects 'fixed £' in Electron — percentage discount impossible on desktop | Restaurant EPOS | Medium | 🔴 Open | Krit | 2026-06-03 |
