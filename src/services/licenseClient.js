...
// SEPOS-060 phase 2 — desktop OFFLINE license lock (till side).
//
// Runs ONLY on the desktop till (local mode). Periodically checks in with the
// tenant cloud's GET /api/license, verifies the SIGNED token with the bundled
...
//    LICENSE_PRIVATE_KEY is deployed on the tenant backends), the till is NEVER
//    locked. So shipping this build cannot brick a paying till before
//    enforcement is deliberately switched on.
//  • A churned client stops getting fresh tokens, so its cached valid_until runs
//    out the GRACE window (default 14 days) and THEN locks — never instantly,
//    never mid-payment for someone who's been paying.
...
async function checkIn() {
  ...
  try {
    ...
    if (data && data.active === false) {
      // Cloud refuses to issue a token (churned/cancelled/suspended). Do NOT
      // extend valid_until — the cached token runs out its grace, then locks.
      state.status = data.status || 'churned';
      save();
      return;
    }
